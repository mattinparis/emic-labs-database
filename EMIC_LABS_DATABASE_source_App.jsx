      ...
      <div className="flex items-center gap-3 mt-2">
        ...
        {saved && !dirty && (
          <span className="text-xs flex items-center gap-1" style={{ color: "#27AE60" }}>
            <Check size={13} /> Saved to your workspace
          </span>
        )}
      </div>
    </div>
...
function ProductionDetail({ production, readOnly, onUpdate, onClose }) {
  ...
  const persist = async (updates) => {
    ...
    await saveJSON(`emic-prod:${production.id}`, next);
  };
...
/* ---------------------------------------------------------------------- */
/*  SCHOOL NOTES, shared and editable. Not access-restricted in this       */
/*  prototype, a real build would gate this behind staff login.            */
/* ---------------------------------------------------------------------- */
function NotesView() {
  ...
  useEffect(() => {
    (async () => {
      const saved = await loadSaved("emic-school-notes");
    ...
    <div>
      <div className="flex items-start gap-2 mb-4 rounded-lg p-3" style={{ background: C.cardAlt, border: `1px solid ${C.line}` }}>
        ...
        <p className="text-xs" style={{ color: C.sub, fontFamily: FONTS.body }}>
          Meant for Matt and team only. This prototype has no login system, so anyone with this file can currently see this tab. A real build would gate it behind staff access.
        </p>
      </div>
      {!loaded ? (
        ...
        <EditableSection
          ...
          onSave={async (v) => {
            setNotes(v);
            await saveJSON("emic-school-notes", v);
          }}
        />
      )}
    </div>
    ...
    (async () => {
      const merged = await Promise.all(
        DEFAULT_PRODUCTIONS.map(async (p) => {
          const saved = await loadSaved(`emic-prod:${p.id}`);
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: `radial-gradient(ellipse 80% 60% at 50% 0%, #171512 0%, ${C.bg} 60%)`, fontFamily: FONTS.body }}>
      ...
      <footer className="text-center text-xs py-6" style={{ color: C.mute, fontFamily: FONTS.body }}>
        Prototype. Each group's tab edits save privately to your own workspace as you go.
      </footer>
